...
class Validaciones:
    ...
    def validando_formulario(self, objeto_app, objeto_conexion, id, nombre, password, apellido, direccion, comentario, consulta):
        if consulta == "CREATE":
            self.llamando_create(objeto_conexion, nombre,
                                 password, apellido, direccion, comentario)
        elif consulta == "READ":
            self.llamando_read(objeto_app, objeto_conexion, id,
                               nombre, password, apellido, direccion, comentario)
        elif consulta == "UPDATE":
            self.llamando_update(objeto_conexion, id, nombre,
                                 password, apellido, direccion, comentario)
        else:
            self.llamando_delete(objeto_conexion, id)

    # funcion para llamar a la funcion de CREATE
    def llamando_create(self, objeto_conexion, nombre, password, apellido, direccion, comentario):
        # creando lista para pasarlelo a la clase crud
        if nombre and password and apellido and direccion and comentario and objeto_conexion.conexion:
    ...
    def llamando_read(self, objeto_app, objeto_conexion, id, nombre, password, apellido, direccion, comentario):
        if objeto_conexion.conexion and (id or nombre or password or apellido or direccion or comentario):
    ...
    def llamando_update(self, objeto_conexion, id, nombre, password, apellido, direccion, comentario) -> None:
        if objeto_conexion.conexion and id and (nombre or password or apellido or direccion or comentario):
            datos = [id, nombre, password, apellido, direccion, comentario]
